...
def get_or_update_hotel(slug):
    if request.method == 'GET':
        hotel = fetch_one("SELECT * FROM hotels WHERE LOWER(slug) = %s", (slug.lower(),))
        if not hotel:
            return jsonify({"error": "Hotel not found"}), 404

        hotel_id = hotel["hotel_id"]

        images = fetch_all("SELECT filename, category, description FROM hotel_images WHERE hotel_id = %s", (hotel_id,))
        rooms = fetch_all("SELECT room_type, bed_type, size_sqm, occupancy, description FROM rooms WHERE hotel_id = %s", (hotel_id,))
        amenities = fetch_all("SELECT amenity_name, description FROM amenities WHERE hotel_id = %s", (hotel_id,))
        dining = fetch_all("SELECT name, cuisine, description FROM dining_options WHERE hotel_id = %s", (hotel_id,))
        spa = fetch_all("SELECT service_name, description FROM spa_services WHERE hotel_id = %s", (hotel_id,))
