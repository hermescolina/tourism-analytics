...
def get_or_update_car(slug):
    if request.method == 'GET':
        ...
        images = fetch_all("SELECT image_id, filename, category, description FROM car_images WHERE car_id = %s", (car_id,))
        ...
        print("🟡 Values:", values)  # This shows the parameter values that are sent to the query
        cursor.execute(query, values)
        conn.commit()
        cursor.close()
        conn.close()
...
def upload_car_image():
    ...
    is_background = request.form.get('is_background') == 'true'  # For background image logic
    ...
    try:
        ...
        return jsonify({"error": "Image already exists for this car and category."}), 409
    finally:
        cursor.close()
        conn.close()
...
def get_tour_history_images(slug):
    try:
        tours_db = mysql.connector.connect(**tours_db_config)  # ✅ establish connection
        ...
        if 'tours_db' in locals(): tours_db.close()
